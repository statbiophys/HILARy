...
class Compatible:
    ...
    def __init__(
        self, filename_v=None, filename_j=None, offset_v=50, offset_j=33, threads=None
    ):
        ...
        aa103 = 3 * (104 - 1)
        self.genes_v["anchor_index"] = aa103 - self.genes_v[
            "germline_alignment"
        ].str.count(r"\.")
        self.genes_j["anchor_index"] = self.genes_j["germline_alignment"].str.find(
            "tgggg"
        )
    ...
    def fasta2df(self, filename):
        ...
        df = pd.DataFrame(
            list(zip(descriptions, seqs)), columns=["label", "germline_alignment"]
        )
        head = ["_"] * 16
        head[1] = "call"
    ...
    def group2airr(self, args):
        ...
        if len(v) == 1 and len(j) == 1:
            ...
            df["v_sequence_alignment"] = df["offset_v_sequence_alignment"].str[
                self.offset_v :
            ]
