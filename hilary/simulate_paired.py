...
from utils import mutate, mutate2
...
class SimulatorPaired():
    ...
    def __init__(self, 
                 threshold:  int = 10, 
                 model_directory_heavy: str ='human_B_heavy',
                 model_directory_light: str ='human_B_kappa') -> None:
    ...
    def simulate(self,
                 ...
                 mut_directory: str | None = None) -> pd.DataFrame:
        ...
        if subtype.lower()=='igg':
            ...
            if mut_directory is None:
                mut_directory: str = 'mutations/326713_igg_'
        elif subtype.lower()=='igm':
            ratios=[1/12,11/12]
            if alpha is None:
                alpha=2.2
            if mut_directory is None:
                mut_directory: str = 'mutations/326713_igm_'
        elif subtype.lower()=='mixture':
            ratios=[1/8,7/8]
            if alpha is None:
                alpha=2.25
            if mut_directory is None:
                mut_directory: str = 'mutations/326713_'
        else:
        ...
        if self.light_simulator.nbLarge > 0: self.familiesLarge['family'] = self.familiesLarge['family'] + 1
        self.familiesSmall=self.smallFamilies()
        self.familiesSmall['family'] = self.familiesSmall['family'] + (1+self.heavy_simulator.nbLarge) 
    ...
    def generate_naive(self,nbOfFamilies):
        ...
        self.rootsLarge_heavy, self.rootsSmall_heavy = self.heavy_simulator.generate_naive(nbOfFamilies)
        self.rootsLarge_light, self.rootsSmall_light = self.light_simulator.generate_naive(nbOfFamilies)
    ...
    def largeFamilies(self):
        ...
        spectra = self.heavy_simulator.large.sample(len(self.rootsLarge_heavy),replace=False).reset_index(drop=True)
        families = pd.DataFrame()
        print('Generate large families.')
        zipped_iter = zip(self.rootsLarge_heavy.iterrows(), self.rootsLarge_light.iterrows())
        for ((index , (v_heavy,j_heavy,l_heavy,_,naive_heavy,anchor_heavy)),
            (index_1,(v_light,j_light,l_light,_,naive_light,anchor_light))) in tqdm(zipped_iter):
            ...
            family = pd.DataFrame()
            family['sequence_heavy'],family['sequence_light'] = mutateFamilyPaired(naive_heavy,naive_light,familySize,which[order],howMany[order])
            ...
            family['cdr3_heavy'] = family['sequence_heavy'].str[anchor_heavy+3:anchor_heavy+3+int(l_heavy)]
            family['cdr3_light'] = family['sequence_light'].str[anchor_light+3:anchor_light+3+int(l_light)]
            family['family'] = index
    ...
    def smallFamilies(self):
        ...
        self.rootsSmall=self.rootsSmall_heavy.merge(self.rootsSmall_light,left_index=True,right_index=True,suffixes=('_heavy', '_light'))
        ...
        for _, (familySize,n_samples) in tqdm(self.rootsSmall.family_size.value_counts().reset_index().iterrows()):
            spectra=self.heavy_simulator.small.loc[self.heavy_simulator.small['family_size']==familySize].reset_index()['mutation_spectrum'].apply(lambda x: str(x).split(':'))
            sampled_spectra = np.random.choice(np.arange(len(spectra)),size=n_samples,replace=True)
            out=pd.DataFrame()
        ...
        cols=['family_size','cdr3_length_heavy','j_gene_heavy','v_gene_heavy','cdr3_length_light','j_gene_light','v_gene_light']
        return applyParallel(self.rootsSmall.groupby(cols),small_family_parallel_paired,cpuCount=cpu_count())
...
def mutateFamilyPaired(naive_heavy,naive_light,familySize,which,howMany):
    ...
    length = len(naive)
    scale = length/250. # Briney IGH templated alignment length is 250
    for w,nb in zip(which,howMany):
        nb_scaled = int(nb*scale) # np.random.poisson(nb*scale)
        xs = np.random.randint(0,length,size=nb_scaled) # here context/position dependence model should enter
        tos = np.random.randint(3,size=nb_scaled) # substitution model enters here
        for x,to in zip(xs,tos):
            for i in w.nonzero()[0]: sequences[i] = mutate(sequences[i],x,mutate2.get(naive[x])[to])
...
def small_family_parallel_paired(args) -> pd.DataFrame:
    ...
    families = pd.DataFrame()
    for index,(v_heavy,j_heavy,cdr3_length_heavy,_,naive_heavy,anchor_heavy,
               v_light,j_light,cdr3_length_light,_,naive_light,anchor_light,_,spectrum) in rootsSmall.iterrows():
        family = pd.DataFrame()
        if sum(spectrum)==0: 
        ...
        family['v_gene_heavy'],family['v_gene_light'] = v_heavy,v_light
        family['j_gene_heavy'],family['j_gene_light'] = j_heavy,j_light
        family['cdr3_length_heavy'],family['cdr3_length_light'] = cdr3_length_heavy,cdr3_length_light
        family['cdr3_heavy'] = family['sequence_heavy'].str[anchor_heavy+3:anchor_heavy+3+int(cdr3_length_heavy)]
        family['cdr3_light'] = family['sequence_light'].str[anchor_light+3:anchor_light+3+int(cdr3_length_light)]
        family['family'] = index
        families = pd.concat([families,family],ignore_index=True)
    return families
...
def produce_alt_alignments_paired(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['cdr3_start_heavy']=df[['sequence_heavy','cdr3_heavy']].apply(lambda x: str(x[0]).find(x[1]),axis=1).astype(int)
    df['cdr3_end_heavy']=df['cdr3_start_heavy']+df['cdr3_length_heavy']
    df['alt_sequence_alignment_heavy']=df[['sequence_heavy','cdr3_start_heavy','cdr3_end_heavy']].apply(lambda x: (x[0][:x[1]]+x[0][x[2]:]),axis=1)
    df['alt_germline_alignment_heavy']=df[['germline_heavy','cdr3_start_heavy','cdr3_end_heavy']].apply(lambda x: (x[0][:x[1]]+x[0][x[2]:]),axis=1)
    df['cdr3_length_light']=df.cdr3_light.apply(len)
    df['cdr3_start_light']=df[['sequence_light','cdr3_light']].apply(lambda x: str(x[0]).find(x[1]),axis=1).astype(int)
    df['cdr3_end_light']=df['cdr3_start_light']+df['cdr3_length_light']
    df['alt_sequence_alignment_light']=df[['sequence_light','cdr3_start_light','cdr3_end_light']].apply(lambda x: (x[0][:x[1]]+x[0][x[2]:]),axis=1)
    df['alt_germline_alignment_light']=df[['germline_light','cdr3_start_light','cdr3_end_light']].apply(lambda x: (x[0][:x[1]]+x[0][x[2]:]),axis=1)
    return df