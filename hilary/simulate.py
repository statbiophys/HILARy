...
class Simulator():
    ...
    def __init__(self,
                 threshold:  int = 10,
                 model_directory: str ='human_B_heavy') -> None:
        ...
        if model_directory in default_models:
            model_directory=os.path.join(os.path.dirname(sonnia.sonia.__file__),'default_models',model_directory)
        self.righor_model =righor.vdj.Model.load_model_from_files(
                                         os.path.join(model_directory,'model_params.txt'),
    ...
    def simulate(self,
        ...
        self.rootsLarge, self.rootsSmall = self.generate_naive(nbOfFamilies,cdr3_selection,available_j=available_j,available_v=available_v)
        self.familiesLarge = self.largeFamilies(self.rootsLarge)
        if self.nbLarge > 0: self.familiesLarge['family'] = self.familiesLarge['family'] + 1
        ...
        self.dataset = self.dataset.loc[self.dataset.sequence.apply(lambda x: '*' not in nt2aa(x))] # get rid of stop codons
        self.dataset = self.dataset.loc[self.dataset.mutation_count.apply(lambda x: x<70)] # get rid of too many mutations
        return self.dataset
    ...
    def generate_naive(self, 
                       ...
                       cdr3_selection: int | None  = None,
                       available_j: Iterable | None = None,
                       available_v: Iterable | None = None) -> (pd.DataFrame, pd.DataFrame):
        """ Sample generated sequences (naive) """
        print('Generate naive sequences.')
        df = generate_ppost_seqs(self.sonia_model,self.righor_model,n_seqs=n,available_j=available_j,available_v=available_v)    
        ...
        df[['sequence','cdr3_anchor']] = df[['v_gene','j_gene','cdr3']].apply(self.assignSequence,axis=1,result_type='expand').dropna()
        return df[:self.nbLarge],df[self.nbLarge:]
    ...
    def smallFamilies(self):
        ...
        for _, (familySize,n_samples) in tqdm(self.rootsSmall.family_size.value_counts().reset_index().iterrows()):
            spectra=self.small.loc[self.small['family_size']==familySize].reset_index()['mutation_spectrum'].apply(lambda x: str(x).split(':'))
        ...
        return applyParallel(self.rootsSmall.groupby(['family_size','cdr3_length','j_gene','v_gene']),small_family_parallel,cpuCount=cpu_count())
...
def mutateFamily(naive,familySize,which,howMany):
    ...
    length = len(naive)
    scale = length/250. # Briney templated alignment length is 250, full sequence is 320+l (on average)
    for w,nb in zip(which,howMany):
        ...
        while stop_codon and k < 10000:
            ...
            for x,to in zip(xs,tos):
                for i in w.nonzero()[0]: seqs[i] = mutate(seqs[i],x,mutate2.get(naive[x])[to])
            stop_codons = ['*' in nt2aa(s) for s in seqs]
            stop_codon = any(stop_codons)
            k+=1
            if k==1000: print('Warning: stop codon not removed')
        sequences = seqs
    return sequences
...
def produce_alt_alignments(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['cdr3_start']=df[['sequence','cdr3']].apply(lambda x: str(x[0]).find(x[1]),axis=1).astype(int)
    df['cdr3_end']=df['cdr3_start']+df['cdr3_length']
    df['alt_sequence_alignment']=df[['sequence','cdr3_start','cdr3_end']].apply(lambda x: (x[0][:x[1]]+x[0][x[2]:]),axis=1)
    df['alt_germline_alignment']=df[['germline','cdr3_start','cdr3_end']].apply(lambda x: (x[0][:x[1]]+x[0][x[2]:]),axis=1)
    df['mutation_count']=df[["alt_sequence_alignment", "alt_germline_alignment"]].apply(lambda x: hamming(*x),axis=1)
    return df